...
class HeroicTheme extends BaseTheme {
  ...
  extendToolbar(toolbar) {
    this.tooltip = new HeroicTooltip(this.quill, this.options.bounds);
    this.tooltip.root.appendChild(toolbar.container);
    this.buildButtons([].slice.call(toolbar.container.querySelectorAll('button')));
    this.buildPickers([].slice.call(toolbar.container.querySelectorAll('select')));
  }
}
HeroicTheme.DEFAULTS = extend(true, {}, BaseTooltip.DEFAULTS, {
  modules: {
    toolbar: {
      handlers: {
        link: function(value) {
...
class HeroicTooltip extends BaseTooltip {
...
HeroicTooltip.TEMPLATE = [
  '<span class="ql-tooltip-arrow"></span>',
  '<div class="ql-tooltip-editor">',
    '<input type="text" data-formula="e=mc^2" data-link="test.com" data-video="Embed URL">',
    '<a class="ql-close"></a>',
  '</div>'
].join('');


export default HeroicTheme;
